...
const Dropdown = (props: Props) => {
  ...
  return (
    <div className={styles.ddWrapper} ref={wrapperRef} style={overWriteStyles?.ddWrapper}>
      <button type="button" className="dd-header" onClick={handleToggle}>
        <div className="dd-header-title">{state.title}</div>
        {state.isOpen ? (
          <p>
            <i className={`${styles.arrow} ${styles.up}`} style={overWriteStyles?.arrowUp}></i>
          </p>
        ) : (
          <p>
            <i className={`${styles.arrow} ${styles.down}`} style={overWriteStyles?.arrowDown}></i>
          </p>
        )}
      </button>
      {state.isOpen && (
        <div className={styles.ddList} style={overWriteStyles?.ddList}>
          {state.data.map((item: DropdownItem) => (
            <button
              type="button"
              className={`${styles.ddListItem} ${item.selected ? styles.activeItem : ''}`}
              key={item.id}
              onClick={() => selectItem(item)}
              style={{
                ...(overWriteStyles.ddListItem ? overWriteStyles.ddListItem : {}),
                ...(item.selected && overWriteStyles.activeItem ? overWriteStyles.activeItem : {}),
